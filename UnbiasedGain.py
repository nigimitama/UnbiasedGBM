...
class logloss_tool:
    def calc_g(self, y, fpre):
        fpre = min(fpre, 350)
        if y == 1:
            return -1.0 / (math.exp(fpre) + 1)
        if y == 0:
            return math.exp(fpre) / (math.exp(fpre) + 1)
        print("Unknown label. binary label must be 0/1")

    def calc_h(self, y, fpre):
        fpre = min(fpre, 350)
        if y == 1:
            return math.exp(fpre) / (math.exp(2 * fpre) + 2 * math.exp(fpre) + 1)
        if y == 0:
            return math.exp(fpre) / (math.exp(2 * fpre) + 2 * math.exp(fpre) + 1)
        print("Unknown label. binary label must be 0/1")
...
def get_left_index(data, dtype, thresh, aim):
    if isinstance(thresh, str):
        ...
        aim = not aim
        if dtype == "==":
            return (data.apply(lambda x: x in thresh)) == aim
        else:
            print(f"Error : unknown str with not ==")
    if dtype == "==":
        return (data == thresh) == aim
    if dtype == "<=":
        return (data <= thresh) == aim
    if dtype == ">=":
        return (data >= thresh) == aim
...
def calcGH(data, samples=-1):
    if samples == -1:
        return np.array(
            [
                data["_G"][data["_istrain"] == True].sum(),
                data["_G"][data["_istrain"] == False].sum(),
            ]
        ), np.array(
            [
                data["_H"][data["_istrain"] == True].sum(),
                data["_H"][data["_istrain"] == False].sum(),
            ]
        )
    else:
        if samples < global_min_samples:
            return 0.0, 0.0, 0.0
...
def gogogo(tree, feat_imp, data, over_all_data, biased):
    ...
    if "left_child" in tree:
        idx_l = get_left_index(
            data[global_feat_list[tree["split_feature"]]],
            tree["decision_type"],
            tree["threshold"],
            tree["default_left"],
        )
        ...
        samples = min(
            len(data_l[data_l["_istrain"] == False]),
            len(data_r[data_r["_istrain"] == False]),
        )
        gl, hl, wlp = calcGH(data_l, samples)
        gr, hr, wrp = calcGH(data_r, samples)
        gx, hx, wxp = calcGH(data, samples)
        score = (
            -gx * wxp + gl * wlp + gr * wrp if samples >= global_min_samples else 0.0
        )
        if biased:
            Gl, Hl = calcGH(data_l)
            Gr, Hr = calcGH(data_r)
...
def convert_str_to_tree(s):
    ...
    def pop_back(stack):
        if "left_child" in stack[-2][1]:
            stack[-2][1]["right_child"] = stack[-1][1]
        else:
            stack[-2][1]["left_child"] = stack[-1][1]
        return stack[:-1]
    ...
    for line in s.split("\n"):
        ...
        if "[" in line:
            ...
            node = {
                "split_feature": global_feat_list.index(keys.split("<")[0]),
                "decision_type": "<",
                "threshold": float(keys.split("<")[1]),
                "default_left": True,
            }
        else:
...
def calc_gain(model, dataT, labelT, dataV, labelV, losstool, biased=False):
    ...
    feat_imp = [0.0] * data.shape[1]
    data["_label"] = labelT.values.tolist() + labelV.values.tolist()
    data["_istrain"] = [True] * len(labelT) + [False] * len(labelV)
    data["_pred"] = [0.0] * len(data)
    if isinstance(model, xgb.sklearn.XGBRegressor) or isinstance(
