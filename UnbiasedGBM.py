...
class UnbiasedTree:
    def __init__(self, booster, p):
        ...
        if int(txt[0]):
            ...
            if self.split_type == "cat":
                self.split_dict[np.nan] = self.split_dict[-1]
    ...
    def apply_split(self, df):
        if self.split_type == "cat":
            ...
            return (
                df[self.split_col].apply(
                    lambda x: (
                        self.split_dict[x]
                        if x in self.split_dict
                        else self.split_dict[np.nan]
                    )
                )
                <= self.split_val
            )
            ...
            return (
                df[self.split_col].apply(
                    lambda x: self.split_dict[-1] if pd.isna(x) else x
                )
                <= self.split_val
            )
    ...
    def predict_single(self, df):
        ...
        return self.child[is_right].predict_single(df) + (
            self.split_avg_r if is_right else self.split_avg_l
        )
...
class UnbiasedBoost:
    ...
    def fit(
        ...
        if self.losstool == "MSE":
            txt = (
                f"regression MSE {self.n_est} {self.min_leaf} {self.thresh} {self.lr}\n"
            )
        if txt is None:
